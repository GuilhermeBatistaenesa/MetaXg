...
class OutputManager:
    ...
    def _write_public_text(self, kind: str, filename: str, content: str, when: datetime):
        if not self.public_base_dir:
            return
        try:
            base = self._public_base_dir(kind)
    ...
    def append_text(self, kind: str, filename: str, content: str, when: datetime = None) -> str:
        when = when or self.started_at
        local_dir = self._local_base_dir(kind)
        local_path = os.path.join(local_dir, filename)
        existing = ""
        ...
        self._atomic_write_text(local_path, new_content)
        self._write_public_text(kind, filename, new_content, when)
        return local_path
